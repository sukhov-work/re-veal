...
Range: T1–T13 · 13 rows · 8 OPEN · 1 DEFERRED (T6) · 4 CLOSED (T4, T7, T9, T12) (update this line at every state change; the range must have no gaps).
...
| T13 | 2026-09-14 | Class B morph shows the warped finish frame's rectangular border walking through the picture on every mismatched fixture (owner: "you can see next frame square borders which is especially ugly in transition"); the similarity warp moves the whole frame and the hole fill exposes its edge | `transitions.py` `morph_frame` / `forward_splat` hole fill; `benchmarks/2026-09-13-real-pairs.md §Owner picks` | OPEN (2026-09-14) — slice TR2c, first in the queue |
