...
| 2026-09-23 | **`models/` gains a second writer.** `transitions.py warmup` fetches Depth Anything V2 Small (99 MB, Apache-2.0) into `models/depth/` (the transitions tool's own `HF_HOME`) and records `models/DEPTH_MANIFEST.json`, mirroring decisions 24–27: manifest-gated, refuses to download at runtime, a real forward pass at warmup, sha256 of every file. Reveal's `models/hub/checkpoints` and `MANIFEST.json` are untouched and `reveal.py` is byte-identical. The depth cache and its manifest are gitignored (the safetensors file is 99,173,660 bytes, near GitHub's 100 MB limit); `./setup.sh --learned` rebuilds them. | DECISIONS 2026-09-23; `TRANSITIONS.md §1–2`; transitions harness 48–51 |
