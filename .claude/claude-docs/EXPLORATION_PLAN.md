...
| **TR2** real pairs (research E2; rank 2) | ten catalogued pairs × {morph, flow-dissolve, snap-morph} × {1 s, 3 s}: one sheet of `report.json` rows + strips; class routing right on every related pair; `edge_ratio` ≤ 1.5 on the smooth presets; owner rates ≥ 6/10 "usable as-is" | a bench script over `fixtures/MANIFEST.md` (shares H2's catalogue) | `[revert: commit]` | the sheet reproduces on a second run | M | T4 (done) | **first sheet 2026-09-13**, 12 pairs (`benchmarks/2026-09-13-real-pairs.md`); routing 11/12 as labelled (mismatch_7 shares its skyline), `edge_ratio` gate green; owner rating pending |
